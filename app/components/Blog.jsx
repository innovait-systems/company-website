...
export const blogPosts = [
  {
    slug: "real-time-fleet-tracking-2000-vehicles",
  ...
  {
    slug: "saas-multi-tenancy-before-enterprise",
    category: "Product",
  ...
  {
    slug: "llms-in-the-enterprise-what-works",
    category: "AI & Tech",
...
export default function Blog() {
  ...
  return (
    <section id="blog" className="relative py-32 bg-page overflow-hidden" ref={ref}>
      <div
        className="absolute top-0 right-0 w-1/3 h-2/3 opacity-5 pointer-events-none"
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {blogPosts.map((p, i) => (
            <Link
              key={p.slug}
              href={`/blog/${p.slug}`}
              className={`group block transition-transform duration-500 ${
                visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              }`}
              style={{ transitionDelay: `${i * 130}ms`, transitionDuration: "700ms" }}
            >
              <article className="card-dark relative overflow-hidden rounded-xl h-full hover:-translate-y-1.5 hover:shadow-2xl transition-transform duration-500">
              {/* Top accent bar */}
              <div
              ...
              <div className="p-7 md:p-8 flex flex-col h-full">
                ...
                {/* Title */}
                <h3 className="font-display font-bold text-white text-base md:text-lg leading-snug mb-3 group-hover:text-gold transition-colors duration-300">
                  {p.title}
                </h3>

                {/* Excerpt */}
                <p className="text-white/40 font-body text-sm leading-relaxed mb-5 line-clamp-3">
                  {p.excerpt}
                </p>
                ...
                <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                  ...
                  <div
                    className="flex items-center gap-1 text-xs font-body opacity-0 group-hover:opacity-100 transition-all duration-300"
                    style={{ color: p.accent }}
                  >
                    <span>Read</span>
